...
def _google_book():
    global _GSPREAD_CLIENT, _GSPREAD_BOOK

    if _GSPREAD_BOOK is not None:
        return _GSPREAD_BOOK

    if not GOOGLE_SERVICE_ACCOUNT_JSON:
        raise RuntimeError("GOOGLE_SERVICE_ACCOUNT_JSON табылган жок / не найден")
    ...
    _GSPREAD_BOOK = _GSPREAD_CLIENT.open_by_key(SHEET_ID)
    return _GSPREAD_BOOK

def _worksheet(sheet_name):
    return _google_book().worksheet(sheet_name)

def read_sheet(sheet_name):
    ws = _worksheet(sheet_name)
    # numericise_ignore=['all'] бардык маанини текст түрүндө калтырат.
...
def bind_telegram_id_in_sheet(customer_code, telegram_id):
    """Кардар кодун Telegram ID менен Google Sheet'ке туруктуу байлайт."""
    customer_code = str(customer_code or "").strip().upper()
    telegram_id = str(telegram_id or "").strip()
    if not customer_code or not telegram_id:
        raise ValueError("customer_code/telegram_id бош")

    ws = _worksheet(CUSTOMERS_SHEET)
    values = ws.get_all_values()
    if not values:
        raise RuntimeError("Кардарлар барагы бош")

    headers = [str(v).strip() for v in values[0]]
    if "Кардар коду" not in headers:
        raise RuntimeError("Кардар коду колонкасы табылган жок")

    code_col = headers.index("Кардар коду") + 1
    telegram_col = headers.index("Telegram ID") + 1 if "Telegram ID" in headers else 0
    if not telegram_col:
        telegram_col = len(headers) + 1
        ws.update_cell(1, telegram_col, "Telegram ID")

    found_row = None
    for idx, row in enumerate(values[1:], start=2):
        code = str(row[code_col - 1] if len(row) >= code_col else "").strip().upper()
        if code == customer_code:
            found_row = idx
            break

    if not found_row:
        raise RuntimeError("Кардар табылган жок")

    current = str(ws.cell(found_row, telegram_col).value or "").strip()
    if current and current != telegram_id:
        return False

    if not current:
        ws.update_cell(found_row, telegram_col, telegram_id)
    return True
...
def profile_by_customer_code(message):
    ...
    try:
        ...
        # Кардар кодун Google Sheet'ке Telegram ID менен туруктуу байлайбыз.
        if not bind_telegram_id_in_sheet(customer_code, message.from_user.id):
            text = (
                "⛔ Бул кардар коду башка Telegram аккаунтуна байланыштырылган."
                if lang == "kg"
                else "⛔ Этот код клиента уже привязан к другому Telegram-аккаунту."
        ...
        # Локалдык байланыш тез резерв катары калат.
        bind_code(message.from_user.id, customer_code)

        # Жаңы Telegram ID таблицага жазылгандан кийин профилди көрсөтөбүз.
        customer = customer_by_code(customer_code) or customer
        show_profile(message, customer)
...
def show_parcels(message, customer):
    ...
    for i, row in enumerate(rows, 1):
        ...
        payment_number = _parse_number(payment_som)
        if payment_number is not None:
            total_payment += payment_number
    ...
    if lang == "ru":
        ...
        footer = (
            f"\n\n📊 Позиция: {len(rows)}"
            f"\n⚖️ Вес посылок: {total_weight:.2f} кг"
            f"\n💰 Стоимость: {format_som(total_payment)} сом"
        )
    else:
        header = f"👤 Кардар: {name}\n🆔 Код: {customer_code}\n\n"
        footer = (
            f"\n\n📊 Позиция: {len(rows)}"
            f"\n⚖️ Посылкалардын салмагы: {total_weight:.2f} кг"
            f"\n💰 Баасы: {format_som(total_payment)} сом"
        )
